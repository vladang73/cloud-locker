...
import { UpdateUserForm, AccountRole, UserStatus } from 'types'
...
export default function EditClientForm() {
  ...
  useEffect(() => {
    ...
    if (data.isSuccess || data.isError) {
      manageUsers.clientEditModalOpen = false
    }
  ...
  const schema = yup.object().shape({
    ...
    role: yup.mixed().oneOf(['client-user']),
  })

  const { register, control, handleSubmit, errors, watch } = useForm<UpdateUserForm>({
    resolver: yupResolver(schema),
    shouldUnregister: false,
  })

  const roles: { role: AccountRole; title: string }[] = [
    { role: 'client-user', title: 'Client User' },
  ]
  ...
  const onSubmit = (input: UpdateUserForm) => {
    data.updateUser().then(() => {
      ...
      >
        <Grid container justify="center">
          {!data.showCases ? (
            ...
            <>{data.role === 'client-user' && <CasePermissions data={data} />}</>
          )}
