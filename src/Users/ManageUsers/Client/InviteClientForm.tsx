...
export default function InviteClientForm() {
  ...
  const clientContainer = useRef<InviteUserContainer>(new InviteUserContainer(axios, 'client-user'))
  const data = useObserver(clientContainer.current)
  ...
  useEffect(() => {
    ...
    if (data.isSuccess || data.isError) {
      manageUsers.clientInviteModalOpen = false
    }
