...
export function PersonalGroup(props: PersonalGroupProps) {
  ...
  const handleClick = () => {
    let optionList: CollapseOption = {
      workgroup: false,
      personal: !collapseOption.personal,
      share: false,
      recycle: false,
    }
    ...
    if (folderPath.charAt(0) !== 'p') history.push(`${WORKSPACE_URL}/p/`)
  }
    ...
    <List component="nav" className={classes.root}>
      <ListItem button className={classes.listButton}>
        <ListItemIcon className={classes.listTitleIcon} onClick={handleRootClick}>
          <UserIcon className={classes.primaryIcon} />
        </ListItemIcon>
        <ListItemText
          primary="Personal"
          className={collapseOption.personal ? classes.primaryActiveText : classes.primaryText}
          onClick={handleRootClick}
        />
        {collapseOption.personal ? (
      ...
      <Collapse
        in={collapseOption.personal && isMenuCollapse}
        timeout="auto"
        unmountOnExit
        className={classes.listCollapse}
      >
        <div className={classes.treeContainer}>{collapseOption.personal && <WorkspaceTree />}</div>
      </Collapse>
